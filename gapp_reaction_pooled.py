...
wanting_hyperpar_plots = False
wanting_set_timing = False
...
for set_num in data_sets:
    ...
    if __name__ == '__main__':
        with Pool() as p:
            ...
            for k in range(Tmax):
                ...
                # univariate iteration / revival
                iter_part = partial(f.iterate,y=yk,G=Gk,k=k)
                revive = partial(f.revival,y=yk,k=k)
    ...
    if saving_results and __name__ == '__main__':
        ...
        name = f'Results/GaPP_ReaCtion{set_num}.npz'
